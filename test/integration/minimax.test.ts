...
const config: MockMindConfig = {
  ...
  models: [{ id: "abab6.5s-chat", provider: "minimax" }],
  defaults: { latencyMs: 0, streamChunkDelayMs: 0 },
  fallback: { enabled: true, response: { type: "text", content: "fallback" } },
  scenarios: [
    { id: "minimax", provider: "minimax", endpoint: "/v1/text/chatcompletion_v2", priority: 0, match: { model: "abab6.5s-chat" }, response: { type: "text", content: "minimax" } },
...
describe("MiniMax routes", () => {
  it("returns MiniMax native chat completion", async () => {
    ...
    const response = await app.inject({ method: "POST", url: "/v1/text/chatcompletion_v2", payload: { model: "abab6.5s-chat", messages: [{ role: "user", content: "hello" }] } });
  ...
  it("returns MiniMax provider-style error", async () => {
    const { app } = await createMockMindServer(config);
    const response = await app.inject({ method: "POST", url: "/v1/text/chatcompletion_v2", payload: { model: "abab6.5s-chat", messages: [{ role: "user", content: "error" }] } });
