...
function exampleFor(route) {
  ...
  if (route.protocol === 'openai-responses') return { ...openAIExample(route, docsUrl, { model, input: 'hello' }, { id: 'resp_mock_0001', object: 'response', output_text: 'This is a mock OpenAI Responses API response.' }, ['model', 'input']), stream: streamExampleFor(route, { model, input: 'hello' }, ['event: response.created', 'data: {"type":"response.created"}', '', 'event: response.output_text.delta', 'data: {"type":"response.output_text.delta","delta":"hello"}', '', 'data: [DONE]'].join(nl)) };
  ...
  if (route.protocol === 'anthropic-messages') return { docsUrl, headers: ['x-api-key', 'anthropic-version', 'Content-Type'], required: ['model', 'max_tokens', 'messages'], requestBody: { model, max_tokens: 128, messages: [{ role: 'user', content: 'hello' }] }, responseBody: { id: 'msg_mock_0001', type: 'message', role: 'assistant', content: [{ type: 'text', text: 'Hello from mock Anthropic.' }], stop_reason: 'end_turn' }, curl: curl(route.path, { model, max_tokens: 128, messages: [{ role: 'user', content: 'hello' }] }, ['x-api-key: test-key', 'anthropic-version: 2023-06-01', 'Content-Type: application/json']), stream: { curl: curl(route.path, { model, max_tokens: 128, stream: true, messages: [{ role: 'user', content: 'hello' }] }, ['x-api-key: test-key', 'anthropic-version: 2023-06-01', 'Content-Type: application/json']), responseText: ['event: message_start', 'data: {"type":"message_start"}', '', 'event: content_block_delta', 'data: {"type":"content_block_delta","delta":{"type":"text_delta","text":"hello"}}', '', 'event: message_stop', 'data: {"type":"message_stop"}'].join(nl) } };
  if (route.protocol === 'gemini-generate-content') return { docsUrl, headers: ['Content-Type'], required: ['contents'], requestBody: { contents: [{ role: 'user', parts: [{ text: 'hello' }] }] }, responseBody: { candidates: [{ content: { role: 'model', parts: [{ text: 'Hello from mock Gemini.' }] }, finishReason: 'STOP' }] }, curl: curl(route.path.replace(':modelAndMethod', 'gemini-1.5-pro:generateContent'), { contents: [{ role: 'user', parts: [{ text: 'hello' }] }] }, ['Content-Type: application/json']), stream: { curl: curl(route.path.replace(':modelAndMethod', 'gemini-1.5-pro:streamGenerateContent'), { contents: [{ role: 'user', parts: [{ text: 'hello' }] }] }, ['Content-Type: application/json']), responseText: '[{"candidates":[{"content":{"parts":[{"text":"Hello"}]}}]}]' } };
