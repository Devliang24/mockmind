...
| Azure OpenAI / Microsoft Foundry | Chat Completions | `POST /openai/deployments/:deployment/chat/completions` | `api-key` | `messages`; `model` optional when deployment is present | `stream: true` returns OpenAI-compatible SSE chunks | [Azure OpenAI Chat Completions](https://learn.microsoft.com/azure/ai-services/openai/reference) | Supports deployment-style path and `api-version` query, e.g. `2024-12-01-preview`. |
| Azure OpenAI / Microsoft Foundry | Responses | `POST /openai/v1/responses` | `api-key` | `model`, `input` | `stream: true` returns Responses-style SSE events | [Azure OpenAI API v1](https://learn.microsoft.com/azure/ai-foundry/openai/latest) | Supports v1 Responses path and `api-version=preview` query. |
